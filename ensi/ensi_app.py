...
import os, base64
from ensi.llm_prompts import gerar_sumario, traduzir, organizar
...
def status_inicial():
    ...
    if 'texto_menu_traduzido' not in st.session_state:
        st.session_state.texto_menu_traduzido = ""
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.sumario_artigo:
            ...
            if st.button("Organizar", use_container_width=True):
                st.session_state.texto_menu = organizar(st.session_state.texto_menu)

            if st.button("Traduzir", use_container_width=True):
                st.session_state.texto_menu_traduzido = traduzir(st.session_state.texto_menu)
    ...
    if st.session_state.menu_selecionado and (st.session_state.menu_selecionado != "Selecione um menu..."):
        ...
        col_texto_original, col_texto_tratuzido = st.columns(2)

        with col_texto_original:
            st.subheader("Texto Original")
            st.markdown(st.session_state.texto_menu)

        with col_texto_tratuzido:            
            st.markdown(st.session_state.texto_menu_traduzido)
